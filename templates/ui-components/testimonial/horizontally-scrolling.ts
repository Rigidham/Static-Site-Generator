  ...
  template: `
    <section class="bg-surface-0 dark:bg-surface-950 py-20 px-8 md:px-12 lg:px-20">
      <div class="flex flex-col items-center gap-6 mb-16">
        <h1 class="text-4xl md:text-5xl font-semibold text-surface-900 dark:text-surface-0 leading-tight text-center">Take a look at user reviews</h1>
      ...
      <div class="relative overflow-hidden">
        <div class="hidden sm:block absolute top-0 left-0 w-[15.43rem] h-full z-10 pointer-events-none bg-linear-to-r from-surface-0 dark:from-surface-950 via-surface-0/80 dark:via-surface-950/80 to-transparent"></div>
        <div class="hidden sm:block absolute top-0 right-0 w-[15.43rem] h-full z-10 pointer-events-none bg-linear-to-l from-surface-0 dark:from-surface-950 via-surface-0/80 dark:via-surface-950/80 to-transparent"></div>
        <div #scrollContainer class="overflow-x-auto hide-scrollbar">
          <div class="flex flex-col gap-8">
            <div class="flex gap-8 min-w-max sm:px-[15.43rem]">
              @for (testimonial of firstRowDoubled(); track testimonial.uniqueId) {
                <div class="max-w-88 sm:max-w-104 w-full shrink-0 p-4 sm:p-8 border border-surface-200 dark:border-surface-700 rounded-xl">
            ...
            <div class="flex gap-8 min-w-max sm:px-[15.43rem] sm:translate-x-52">
              @for (testimonial of secondRowDoubled(); track testimonial.uniqueId) {
                <div class="max-w-88 sm:max-w-104 w-full shrink-0 p-4 sm:p-8 border border-surface-200 dark:border-surface-700 rounded-xl">
                  <div class="flex flex-col gap-4">
                    <svg class="w-12 h-12" xmlns="http://www.w3.org/2000/svg" width="40" height="41" viewBox="0 0 40 41" fill="none">
                      <path
...
export class __CLASS_NAME__ implements AfterViewInit {
  ...
  firstRowDoubled = computed(() => [
    ...this.firstRowTestimonials.map((testimonial) => ({ ...testimonial, uniqueId: `first-${testimonial.id}` })),
    ...this.firstRowTestimonials.map((testimonial) => ({ ...testimonial, uniqueId: `first-duplicate-${testimonial.id}` })),
  ]);

  secondRowDoubled = computed(() => [
    ...this.secondRowTestimonials.map((testimonial) => ({ ...testimonial, uniqueId: `second-${testimonial.id}` })),
    ...this.secondRowTestimonials.map((testimonial) => ({ ...testimonial, uniqueId: `second-duplicate-${testimonial.id}` })),
  ]);
