  ...
  template: `
    <div class="bg-surface-0 dark:bg-surface-950 p-12 lg:p-20">
      <div class="flex flex-col gap-12">
        <h1 class="text-2xl font-bold text-surface-900 dark:text-surface-0 leading-tight">Frequently Asked Questions</h1>

        <div class="flex flex-col gap-8">
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            @for (item of faqs; track item.title) {
              <div class="flex flex-col gap-4 p-8 bg-surface-0 dark:bg-surface-950 border border-surface-200 dark:border-surface-700 rounded-xl">
                <div class="text-lg font-medium text-surface-900 dark:text-surface-0 leading-normal">
                  {{ item.title }}
                </div>
                <p class="text-base text-surface-600 dark:text-surface-400 leading-normal">
                  {{ item.description }}
                </p>
              </div>
            }
