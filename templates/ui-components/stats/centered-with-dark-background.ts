...
@Component({
  ...
  template: `
    <div class="bg-surface-900 p-20 flex flex-col justify-center items-center gap-10">
      <div class="flex flex-col items-center gap-4 max-w-2xl">
        <div class="p-4 bg-primary rounded-full flex justify-start items-start">
        ...
        <div class="text-surface-0 text-4xl font-bold leading-tight text-center">Biggest Tech Company of Region</div>
        <div class="text-surface-400 text-lg font-normal leading-normal text-center">
          Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Consequat semper viverra nam libero justo laoreet sit.
        </div>
      </div>

      <div class="shadow-sm rounded-xl flex md:flex-row flex-col justify-center items-start gap-8">
        @for (item of stats; track item.label; let index = $index) {
          <div class="flex justify-center items-center gap-2">
            <div class="text-surface-0 text-3xl font-bold leading-tight">{{ item.count }}</div>
            <div class="text-surface-400 text-lg font-medium leading-normal">{{ item.label }}</div>
          </div>
        }
      </div>
    </div>
